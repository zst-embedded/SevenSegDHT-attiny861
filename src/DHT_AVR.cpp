...
int8_t DHT::read11(volatile uint8_t *PORT, volatile uint8_t *DDR, uint8_t bit)
{
    // READ VALUES
    int8_t result = _readSensor(PORT, DDR, bit, DHTLIB_DHT11_WAKEUP, DHTLIB_DHT11_LEADING_ZEROS);
...
int8_t DHT::read(volatile uint8_t *PORT, volatile uint8_t *DDR, uint8_t bit)
{
    // READ VALUES
    int8_t result = _readSensor(PORT, DDR, bit, DHTLIB_DHT_WAKEUP, DHTLIB_DHT_LEADING_ZEROS);
...
int8_t DHT::_readSensor(volatile uint8_t *PORT, volatile uint8_t *DDR, uint8_t bit, uint8_t wakeupDelay, uint8_t leadingZeroBits)
{
    ...
    *DDR &= ~(1<<bit);
    ...
    for (uint8_t i = 40; i != 0; )
    {
        ...
        if (state == LOW && pstate != LOW)
        {
            ...
            {
                zeroLoop = min_u16(zeroLoop, loopCount);
